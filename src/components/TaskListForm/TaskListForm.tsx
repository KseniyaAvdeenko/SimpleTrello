...
import {createTaskList} from "../../store/actions/taskListAction";
import {IBoard} from "../../interface/IBoard";

interface ITaskListFormProps {
    taskListForm: boolean;
    setTaskListForm: Function;
    board: IBoard;
}

const TaskListForm: FC<ITaskListFormProps> = ({taskListForm, setTaskListForm, board}) => {
    const dispatch = useAppDispatch();

    const [closeBtnFill, setCloseBtnFill] = useState<string>('#1e1e1e');
    const [newTaskList, setNewTaskList] = useState<string>('')

    const onSubmit = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        dispatch(createTaskList({
            taskListName: newTaskList,
    ...
    return (
        <form onSubmit={e => onSubmit(e)}
              className={taskListForm ? [styles.taskListForm, styles.taskListForm_open].join(' ') : styles.taskListForm}>
            <div className={styles.inputContainer}>
                <input
                    ...
                    value={newTaskList}
                    style={{borderColor: board.background}}
                    onChange={e => setNewTaskList(e.target.value)}
                />
            </div>
            <div className={styles.btnContainer}>
                <button type='submit' className={styles.taskListForm__button}
                        style={{backgroundColor: board.background, color: board.textColor}}>Add List
                </button>
                <svg width="14" height="14"
                     viewBox="0 0 14 14" fill="none"
                     xmlns="http://www.w3.org/2000/svg"
                     onMouseOver={() => setCloseBtnFill(board.background)}
                     onMouseOut={() => setCloseBtnFill('#1e1e1e')}
                     onClick={() => setTaskListForm(false)}
                >
                    <path
